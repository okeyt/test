...
class Util_Ftp
{
    ...
    function connect ()
    {
        if (! empty($this->hostname) && ! empty($this->port)) {
            if ($this->conn = @ftp_connect($this->hostname, $this->port, $this->timeout)) {
                ...
                if ($this->debug) {
                    die(Util_Tools::displayError($this->hostname . ':' . $this->port . '无法连接'));
                }
            }
        } else {
            if ($this->debug) {
                die(Util_Tools::displayError('缺少FTP信息'));
            }
        }
    ...
    function login ()
    {
        if ($this->is_connected && ! empty($this->username) && ! empty($this->password)) {
            if (@ftp_login($this->conn, $this->username, $this->password)) {
                ...
                if ($this->debug) {
                    die(Util_Tools::displayError($this->username . ':' . $this->password . '无法登陆'));
                }
            }
        } else {
            if ($this->debug) {
                die(Util_Tools::displayError('缺少FTP账号信息'));
            }
        }
    ...
    function cd ($path)
    {
        if ($this->is_connected && ! empty($path)) {
            if (@ftp_chdir($this->conn, $path)) {
                ...
                if ($this->debug) {
                    die(Util_Tools::displayError('无法切换目录:' . $path));
                }
            }
        } else {
            if ($this->debug) {
                die(Util_Tools::displayError('FTP未连接或目录为空:' . $path));
            }
        }
    ...
    function mkdir ($path, $permission = null)
    {
        if ($this->is_connected && ! empty($path)) {
            if (@ftp_mkdir($this->conn, $path)) {
                ...
                if ($this->debug) {
                    die(Util_Tools::displayError('无法创建目录:' . $path));
                }
            }
        } else {
            if ($this->debug) {
    ...
    function mv ($old_name, $new_name)
    {
        if ($this->is_connected && ! empty($new_name) && ! empty($old_name)) {
            if (@ftp_rename($this->conn, $old_name, $new_name)) {
                ...
                if ($this->debug) {
                    die(Util_Tools::displayError('FTP MV失败'));
                }
            }
        } else {
            if ($this->debug) {
                die(Util_Tools::displayError('FTP未连接或文件名为空:' . $old_name));
            }
        }
        ...
        if ($this->is_connected && ! empty($path)) {
            if (function_exists('ftp_chmod')) {
                if (@ftp_chmod($this->conn, $permission, $path)) {
                    ...
                    if ($this->debug) {
                        die(Util_Tools::displayError('无法修改目录属性:' . $path . ':' . $permission));
                    }
                }
            } else {
                if ($this->debug) {
                    die(Util_Tools::displayError('无法使用ftp_chmod函数'));
                }
            }
        } else {
            if ($this->debug) {
    ...
    function rmfile ($path)
    {
        if ($this->is_connected && ! empty($path)) {
            if ($result = @ftp_delete($this->conn, $path)) {
                ...
                if ($this->debug) {
                    die(Util_Tools::displayError('无法删除文件:' . $path));
                }
            }
        } else {
            if ($this->debug) {
    ...
    function rmdir ($path)
    {
        if ($this->is_connected && ! empty($path)) {
            ...
            if (@ftp_rmdir($this->conn, $path)) {
                ...
                if ($this->debug) {
                    die(Util_Tools::displayError('FTP删除目录失败:' . $path));
                }
            }
        } else {
            if ($this->debug) {
    ...
    function upload ($file, $remote_path, $mode = 'auto', $permission = null)
    {
        if ($this->is_connected && ! empty($remote_path)) {
            if (file_exists($file)) {
                if ($mode == 'auto') {
                    $mode = $this->_settype(Util_Tools::getFileExtension($file));
                }
                ...
                if (ftp_put($this->conn, $remote_path, $file, $mode)) {
                    ...
                    if ($this->debug) {
                        die(Util_Tools::displayError('FTP上传失败:' . $file . ':' . $remote_path));
                    }
                }
            } else {
                if ($this->debug) {
                    die(Util_Tools::displayError('本地文件不存在:' . $file));
                }
            }
        } else {
            if ($this->debug) {
