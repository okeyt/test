...
class Util_ImageCombiner
{
    ...
    private function checkSrcs ()
    {
        foreach ($this->srcs as $key => $src) {
            ...
            if (Util_Tools::saveFile($src, $tmp)) {
                ...
                if ($src_width && $src_height && $type && in_array($type, array(
                    IMAGETYPE_BMP,
                    IMAGETYPE_JPEG,
                    IMAGETYPE_PNG,
                    IMAGETYPE_GIF
        ...
        if (! $this->done) {
            ...
            if (! empty($this->src_infos)) {
                ...
                if ($this->dest_width > 20000 || $this->dest_height > 20000)
                    throw new Exception('combined image size too large may cause out of memory exception');
                
                $this->canvas = Util_ImageManager::createTransImage($this->dest_width, $this->dest_height);
                switch ($this->combine_mode) {
                    case self::COMBINE_HORIZONTAL:
    ...
    public function saveCombination ($dest_file)
    {
        $image = $this->combine();
        if ($image) {
            if (Util_ImageManager::write($this->dest_type, $this->canvas, $dest_file, 90)) {
                imagedestroy($this->canvas);
